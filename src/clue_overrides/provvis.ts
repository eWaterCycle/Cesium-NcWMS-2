...
class StateRepresentation {
  ...
  selected = false;
  parent: StateRepresentation = null;
  children: StateRepresentation[] = [];
  ...
  build(lookup: { [id: number]: StateRepresentation }, line: StateRepresentation[]) {
  ...
  static toStateRepresentation(graph: provenance.ProvenanceGraph, highlight: any, options: any = {}): StateRepresentation[] {
    //assign doi
    const lookup: any = {};
    ...
    const toState = (s) => {
      const r = new StateRepresentation(s, graph);
      const a = s.creator;
  ...
  private static layout(states: StateRepresentation[], line: StateRepresentation[]) {
    //horizontally align the line
    let byLevel: StateRepresentation[][] = [];
  ...
  static toIcon(repr: StateRepresentation) {
    if (!repr.a) {
  ...
  static render($elem: d3.Selection<StateRepresentation>) {
    $elem
      ...
      .classed('endpoint', (d) => d.children.length === 0)
      .attr('data-doi', (d) => d.doi)
      .attr('title', (d) => d.name);

    $elem.select('span.icon').html(StateRepresentation.toIcon);
    ...
    $elem.select('div.sthumbnail')
      ...
      .style('background-image', (d) => d.s.thumbnail);
  ...
  showDialog() {
    const d = this;
    const icon = StateRepresentation.toIcon(d);
...
export class LayoutedProvVis extends vis.AVisInstance implements vis.IVisInstance {
  ...
  private onStateAdded = (event: any, state: provenance.StateNode) => {
    state.thumbnail = `url('https://avatars2.githubusercontent.com/u/21689003?s=200&v=4')`;

    // event.target = ProvenanceGraph
    const viewObjects: any[] = event.target.objects.map((obj) => obj.value);
    viewObjects.forEach((obj) => {
      if (obj.makeScreenshot) {
        const img = obj.makeScreenshot();
        state.thumbnail = `url(${img})`;
      }
    });
  ...
  private onSelectionChanged = (event: any, type: string, act: ranges.Range) => {
    ...
    this.$node.selectAll('div.state').classed('phovea-select-' + type, function (d: StateRepresentation) {
  ...
  public option(name: string, val?: any) {
  ...
  private build($parent: d3.Selection<any>) {
    ...
    jp.find('.btn-collapse').on('click', (evt) => {
      evt.preventDefault();
      $provenanceGraph.select('.btn-collapse > i').classed('fa-arrow-circle-o-right', $provenanceGraph.classed('collapsed')).classed('fa-arrow-circle-o-left', !$provenanceGraph.classed('collapsed'));
      $provenanceGraph.classed('collapsed', !$provenanceGraph.classed('collapsed'));
    });

    return $provenanceGraph;
  }

  private onStateClick(d: StateRepresentation) {
  ...
  update() {
    ...
    const states = StateRepresentation.toStateRepresentation(graph, this.highlight, { thumbnails: this.options.thumbnails });
    ...
    $states.call(StateRepresentation.render);
  ...
  private updateStoryHighlight() {
    ...
    const $states = this.$node.select('div.states').selectAll<StateRepresentation>('div.state');
