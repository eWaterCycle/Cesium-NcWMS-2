...
export class CLUEWrapper extends EventHandler {
  ...
  constructor(body: HTMLElement, options: ICLUEWrapperOptions = {}) {
    ...
    this.graph.then((graph) => {
      ...
      select('#attachScreenshot').on('click', () => {
       const canvasElement = <HTMLCanvasElement>(document.querySelector('main *[data-main]') || document.querySelector('main'));
       console.log(canvasElement.getBoundingClientRect());
      //  const bounds = C.bounds(canvasElement);
      //  this.header.wait();
